...
export class UserDataExtractor {
  ...
  static sanitizeUserData(
    ...
    const sanitizeString = (
      ...
      const charsToKeep = Math.max(
        1,
        Math.floor(str.length * (percentage / 100))
      )
    ...
    sanitized.firstName = sanitizeString(
      sanitized.firstName,
      sanitizePercentage
    )
    ...
    if (sanitized.email) {
      ...
      if (localPart && domain) {
        const sanitizedLocal =
          sanitizeString(localPart, sanitizePercentage) || localPart
  ...
  static sanitizeJwtToken(token: string, sanitizePercentage = 60): string {
