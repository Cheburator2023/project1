...
export class ConnectionManager {
  ...
  connect(): void {
    ...
    try {
      this.originalConsole.log(
        `[TSLG] Attempting to connect to TSLG agent at ${this.config.host}:${this.config.port} (attempt ${this.connectionAttempts})`
      )
      ...
      this.originalConsole.error(
        '[TSLG] Failed to create TSLG connection:',
        error
      )
  ...
  private setupSocketEventHandlers() {
    ...
    this.socket.on('connect', () => {
      ...
      this.originalConsole.log(
        `[TSLG] Connected successfully to ${this.config.host}:${this.config.port}`
      )
    })
    ...
    this.socket.on('close', (hadError) => {
      this.originalConsole.log(
        `[TSLG] Connection closed${hadError ? ' with error' : ''}`
      )
      if (hadError) {
        this.scheduleReconnect()
      }
  ...
  private calculateReconnectDelay(): number {
    ...
    const delay = Math.min(
      baseDelay * Math.pow(1.5, this.connectionAttempts - 1),
      maxDelay
    )
    return delay
  }
