...
export class SumUsageService extends BaseUsageService implements IUsageService {
  ...
  async handleUpdateUsage(data: UpdateUsageDto): Promise<boolean> {
    this.logger.info('Handling update usage', 'ОбработкаОбновленияИспользования', {
    ...
    try {
      ...
      if (!model) {
        this.logger.warn('Model not found for usage update', 'МодельНеНайденаДляОбновленияИспользования', {
          model_id
        });
        return false
      }
    ...
    // проверить существование записи
      ...
      if (!usage) {
        this.logger.info('Inserting new usage record', 'ДобавлениеНовойЗаписиИспользования', {
          model_id,
          confirmation_year,
          confirmation_quarter
        });
        ...
        if (!usageItem) {
          this.logger.error('Failed to insert usage', 'ОшибкаДобавленияИспользования', null, {
            model_id,
            confirmation_year,
            confirmation_quarter
          });
        ...
        if (this.normalizeToLocalDate(usage.confirmation_date) === this.normalizeToLocalDate(confirmation_date) && usage.confirmed === isUsed) {
          this.logger.info('Usage data unchanged, skipping update', 'ДанныеИспользованияНеИзмененыПропускОбновления', {
            model_id,
            confirmation_year,
            confirmation_quarter
          });
        ...
        this.logger.info('Updating existing usage record', 'ОбновлениеСуществующейЗаписиИспользования', {
          model_id,
          confirmation_year,
          confirmation_quarter
        });
        ...
        if (!usageItem) {
          this.logger.error('Failed to update usage', 'ОшибкаОбновленияИспользования', null, {
            model_id,
            confirmation_year,
            confirmation_quarter
          });
      ...
      if (!usageHistItem) {
        this.logger.error('Failed to insert usage history', 'ОшибкаДобавленияИсторииИспользования', null, {
          model_id,
          confirmation_year,
          confirmation_quarter
        });
      ...
      this.logger.info('Usage update completed successfully', 'ОбновлениеИспользованияУспешноЗавершено', {
        model_id,
        confirmation_year,
        confirmation_quarter
      });
      ...
      this.logger.error('Error handling usage update', 'ОшибкаОбработкиОбновленияИспользования', error, {
        model_id,
        confirmation_year,
        confirmation_quarter
      });
  ...
  async getUsage(params: GetUsageParams): Promise<SUMUsageEntity | null> {
    this.logger.info('Getting usage record', 'ПолучениеЗаписиИспользования', params);
    ...
    try {
      ...
      if (usage) {
        ...
        this.logger.info('Usage record not found', 'ЗаписьИспользованияНеНайдена', params);
      }

      return usage || null
    } catch (error) {
      this.logger.error('Error getting usage record', 'ОшибкаПолученияЗаписиИспользования', error, params);
      throw error;
    }
  }

  async updateUsage(params: UpdateUsageParams): Promise<SUMUsageEntity | null> {
    this.logger.info('Updating usage record', 'ОбновлениеЗаписиИспользования', params);
    ...
    try {
      ...
      if (usage) {
        this.logger.info('Usage record updated successfully', 'ЗаписьИспользованияУспешноОбновлена', {
          model_id: params.model_id,
          confirmation_year: params.confirmation_year,
          confirmation_quarter: params.confirmation_quarter
        });
      } else {
        this.logger.warn('Usage record update returned no rows', 'ОбновлениеЗаписиИспользованияНеВернулоДанных', params);
      }

      return usage || null
    } catch (error) {
      this.logger.error('Error updating usage record', 'ОшибкаОбновленияЗаписиИспользования', error, params);
      throw error;
    }
  }

  async insertUsage(params: InsertUsageParams): Promise<SUMUsageEntity | null> {
    this.logger.info('Inserting usage record', 'ДобавлениеЗаписиИспользования', {
      model_id: params.model_id,
      confirmation_year: params.confirmation_year,
      confirmation_quarter: params.confirmation_quarter
    });
    ...
    try {
      ...
      if (usage) {
        this.logger.info('Usage record inserted successfully', 'ЗаписьИспользованияУспешноДобавлена', {
          model_id: params.model_id,
          confirmation_year: params.confirmation_year,
          confirmation_quarter: params.confirmation_quarter
        });
      } else {
        this.logger.warn('Usage record insert returned no rows', 'ДобавлениеЗаписиИспользованияНеВернулоДанных', params);
      }

      return usage || null
    } catch (error) {
      this.logger.error('Error inserting usage record', 'ОшибкаДобавленияЗаписиИспользования', error, {
        model_id: params.model_id,
        confirmation_year: params.confirmation_year,
        confirmation_quarter: params.confirmation_quarter
      });
  ...
  async insertUsageHistory(params: InsertUsageHistoryParams): Promise<SUMUsageHistEntity | null> {
    this.logger.info('Inserting usage history record', 'ДобавлениеЗаписиИсторииИспользования', {
      model_id: params.model_id,
      confirmation_year: params.confirmation_year,
      confirmation_quarter: params.confirmation_quarter
    });
    ...
    try {
      ...
      if (usageHist) {
        this.logger.info('Usage history record inserted successfully', 'ЗаписьИсторииИспользованияУспешноДобавлена', {
          model_id: params.model_id,
          confirmation_year: params.confirmation_year,
          confirmation_quarter: params.confirmation_quarter
        });
      } else {
        this.logger.warn('Usage history record insert returned no rows', 'ДобавлениеЗаписиИсторииИспользованияНеВернулоДанных', params);
      }

      return usageHist || null
    } catch (error) {
      this.logger.error('Error inserting usage history record', 'ОшибкаДобавленияЗаписиИсторииИспользования', error, {
        model_id: params.model_id,
        confirmation_year: params.confirmation_year,
        confirmation_quarter: params.confirmation_quarter
      });
