...
export class UsageService {
  ...
  async updateUsage(data, source: MODEL_SOURCES): Promise<boolean> {
    ...
    const results = await Promise.all(
      transformedArtefacts.map(artefact => usageService.handleUpdateUsage(artefact))
    )
  ...
  transformArtefacts(
    artefacts: { model_id: string; artefact_tech_label: string; artefact_string_value: string; artefact_value_id: null, creator: string }[]
  ) {
    const currentDate = this.getCurrentDate()
    const result: Record<string, UpdateUsageDto> = {}

    artefacts.forEach(({ model_id, artefact_tech_label, artefact_string_value, creator }) => {
      const match = artefact_tech_label.match(/usage_confirm_(date|flag)_q(\d)/)
      if (!match) return

      const [, type, quarterStr] = match
      const confirmation_quarter = parseInt(quarterStr, 10)
      const confirmation_year = this.determineYear(confirmation_quarter, currentDate)
