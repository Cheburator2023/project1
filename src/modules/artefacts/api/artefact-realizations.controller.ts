...
export class ArtefactRealizationsController {
  ...
  async getByKey(
    ...
    this.logger.info(
      'GET /artefact-realizations/by-key',
      'ЗапросАртефактаПоКлючу',
      {
        model_id,
        artefact_id,
        as_of,
    ...
    if (!model_id || !artefact_id) {
      this.logger.warn(
        ...
        'ОтсутствуютОбязательныеПараметры',
        {
          model_id,
          artefact_id
        }
      ...
      throw new BadRequestException({
        error: {
          ...
          message: 'model_id and artefact_id required'
        }
      })
    }
    ...
    if (include_history !== undefined && include_history !== null) {
      includeHistory =
        include_history.toLowerCase() === 'true' || include_history === '1'
    }

    try {
      const result = await this.service.getByKey({
        model_id,
        artefact_id,
        as_of,
      ...
      if (includeHistory) {
        ...
        if (result === null || result === undefined) {
          this.logger.warn(
            'Service returned null for history request',
            'СервисВернулNullДляИстории',
            {
              model_id,
              artefact_id
            }
        ...
        if (typeof result === 'object' && result !== null) {
          if (!result.hasOwnProperty('history')) {
            this.logger.warn(
              'Service returned object without history property',
              'СервисВернулОбъектБезИстории',
              {
                model_id,
                artefact_id
              }
          ...
          this.logger.warn(
            'Service returned unexpected type for history request',
            'НеожиданныйТипДляИстории',
            {
              model_id,
              artefact_id,
      ...
      if (includeHistory) {
        this.logger.warn(
          'Returning empty history array due to error',
          'ВозвратПустойИсторииИзЗаОшибки'
        )
        return { history: [] }
      }
  ...
  async query(@Body() body: any) {
    ...
    const { model_ids, artefact_ids, as_of, limit, cursor, include_not_found } =
      body || {}

    if (!Array.isArray(model_ids) || !model_ids.length) {
      this.logger.warn(
        'Invalid model_ids parameter',
        'НеверныйПараметрModelIds',
        {
          model_ids
        }
      )
      throw new BadRequestException({
        error: { code: 'INVALID_ARGUMENT', message: 'model_ids required' }
      })
    }

    if (!Array.isArray(artefact_ids) || !artefact_ids.length) {
      this.logger.warn(
        'Invalid artefact_ids parameter',
        'НеверныйПараметрArtefactIds',
        {
          artefact_ids
        }
      )
      throw new BadRequestException({
        error: { code: 'INVALID_ARGUMENT', message: 'artefact_ids required' }
      })
    }
