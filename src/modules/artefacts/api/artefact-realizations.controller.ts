...
export class ArtefactRealizationsController {
  ...
  async getByKey(
    ...
    if (!model_id || !artefact_id) {
      this.logger.warn(
        `Missing required parameters: model_id=${model_id}, artefact_id=${artefact_id}`
      )
    ...
    try {
      ...
      if (includeHistory) {
        ...
        if (result === null || result === undefined) {
          this.logger.warn(
            `Service returned null for history request, returning empty history array`
          )
          return { history: [] }
        }
        ...
        if (typeof result === 'object' && result !== null) {
          ...
          this.logger.warn(
            `Service returned unexpected type for history request: ${typeof result}`
          )
          return { history: [] }
        }
      } else {
        ...
        if (!result) {
          this.logger.warn(
            `No active record found for model_id=${model_id}, artefact_id=${artefact_id}`
          )
