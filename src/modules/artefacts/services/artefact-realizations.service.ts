...
export class ArtefactRealizationsService {
  ...
  async getByKey({
    ...
    try {
      if (include_history) {
        ...
        this.logger.info('Successfully retrieved artefact history', 'УспешноеПолучениеИсторииАртефакта', {
          model_id,
          artefact_id,
        ...
        if (result) {
          this.logger.info('Successfully retrieved active artefact', 'УспешноеПолучениеАктивногоАртефакта', {
            model_id,
            artefact_id
          });
        } else {
          this.logger.warn('No active artefact found', 'АктивныйАртефактНеНайден', {
            model_id,
            artefact_id
          });
        }
      ...
      this.logger.error('Error retrieving artefact', 'ОшибкаПолученияАртефакта', error, {
        model_id,
        artefact_id,
        include_history
      });
  ...
  async query({
    ...
    try {
      ...
      this.logger.info('Query completed successfully', 'УспешноеВыполнениеЗапросаАртефактов', {
      ...
      this.logger.error('Error querying artefacts', 'ОшибкаЗапросаАртефактов', error, {
        model_ids_count: model_ids.length,
        artefact_ids_count: artefact_ids.length
      });
      throw error;
