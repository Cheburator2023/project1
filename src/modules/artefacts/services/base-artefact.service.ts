...
import {
  ARTEFACT_TYPES_REQUIRING_VALUES,
  PSEUDO_ARTEFACTS,
  pseudoArtefactsSourcesRoles
} from '../constants'
import {
  ArtefactEntity,
  ArtefactValueEntity,
...
export abstract class BaseArtefactService implements IArtefactService {
  ...
  async getArtefactWithPermissions(user: UserType) {
    this.logger.info(
      'Getting artefacts with permissions',
      'ПолучениеАртефактовСПравами',
      {
    ...
    try {
      ...
      this.logger.info(
        'Artefacts with permissions retrieved successfully',
        'АртефактыСПравамиУспешноПолучены',
        {
      ...
      this.logger.error(
        'Error getting artefacts with permissions',
        'ОшибкаПолученияАртефактовСПравами',
        error,
        {
          user_groups: user.groups,
          user_name: user.name
  ...
  async getAllArtefactRoles(): Promise<
    Map<number, { sum: string[]; sum_rm: string[] }>
  > {
    this.logger.info(
      'Getting all artefact roles',
      'ПолучениеВсехРолейАртефактов'
    )
    ...
    try {
      ...
      const artefactRolesMap = new Map<
        number,
        { sum: string[]; sum_rm: string[] }
      >()
      ...
      for (const pseudoArtefactSourcesRole of pseudoArtefactsSourcesRoles) {
        if (!artefactRolesMap.has(pseudoArtefactSourcesRole.artefact_id)) {
          artefactRolesMap.set(pseudoArtefactSourcesRole.artefact_id, {
            sum: [],
            sum_rm: []
          })
        }
        artefactRolesMap.get(pseudoArtefactSourcesRole.artefact_id)!.sum =
          pseudoArtefactSourcesRole.roles.sum
        artefactRolesMap.get(pseudoArtefactSourcesRole.artefact_id)!.sum_rm =
          pseudoArtefactSourcesRole.roles.sum_rm
      }

      this.logger.info(
        'Artefact roles retrieved successfully',
        'РолиАртефактовУспешноПолучены',
        {
      ...
      this.logger.error(
        'Error getting artefact roles',
        'ОшибкаПолученияРолейАртефактов',
        error
      )
      throw error
    }
    ...
    try {
      ...
      const cl = (data) =>
        data.reduce((prev, curr) => {
          if (
            !prev.length ||
            prev[prev.length - 1].artefact_id !== curr.artefact_id
          ) {
            curr.values = []
            prev.push(curr)
          }
      ...
      this.logger.error(
        'Error getting artefacts',
        'ОшибкаПолученияАртефактов',
        error
      )
      throw error
    }
  ...
  async handleUpdateArtefact(data: UpdateArtefactDto) {
    this.logger.info(
      'Handling update artefact',
      'ОбработкаОбновленияАртефакта',
      {
    ...
    if (this.isMultiDropdownArtefact(data)) {
      return await this.updateMultiDropdownArtefact(
        data as MultiDropdownArtefact
      )
    } else {
  ...
  private isMultiDropdownArtefact(data: UpdateArtefactDto): boolean {
    const isMulti =
      Array.isArray(data.artefact_string_value) &&
      Array.isArray(data.artefact_value_id)
  ...
  async updateMultiDropdownArtefact(
    artefactData: MultiDropdownArtefact
  ): Promise<boolean> {
    this.logger.info(
      'Updating multi-dropdown artefact',
      'ОбновлениеМультиВыпадающегоАртефакта',
      {
    ...
    try {
      ...
      if (!model) {
        this.logger.warn(
          'Model not found for multi-dropdown artefact update',
          'МодельНеНайденаДляОбновленияМультиАртефакта',
          {
            model_id
          }
        )
      ...
      const artefact: ArtefactEntity | null = await this.getArtefactByTechLabel(
        artefact_tech_label
      )
      if (!artefact) {
        this.logger.warn(
          'Artefact not found for update',
          'АртефактНеНайденДляОбновления',
          {
            artefact_tech_label
          }
        )
      ...
      const latestArtefactRealizations: ArtefactRealizationEntity[] | null =
        await this.getLatestArtefactRealizations(model_id, artefact.artefact_id)
      ...
      const addCandidates = latestArtefactRealizations
        ? differenceById(newArr, latestArtefactRealizations)
        : newArr
      const deleteCandidates = latestArtefactRealizations
        ? differenceById(latestArtefactRealizations, newArr)
        : null

      this.logger.info(
        'Multi-dropdown artefact changes calculated',
        'ИзмененияМультиАртефактаРассчитаны',
        {
          model_id,
      ...
      if (deleteCandidates) {
        this.logger.info(
          'Deleting obsolete artefact realizations',
          'УдалениеУстаревшихРеализацийАртефакта',
          {
      ...
      if (addCandidates.length) {
        this.logger.info(
          'Adding new artefact realizations',
          'ДобавлениеНовыхРеализацийАртефакта',
          {
      ...
      this.logger.info(
        'Multi-dropdown artefact update completed successfully',
        'ОбновлениеМультиАртефактаУспешноЗавершено',
        {
          model_id,
          artefact_tech_label,
          artefact_id: artefact.artefact_id
      ...
      this.logger.error(
        'Error updating multi-dropdown artefact',
        'ОшибкаОбновленияМультиВыпадающегоАртефакта',
        error,
        {
          model_id: artefactData.model_id,
          artefact_tech_label: artefactData.artefact_tech_label
  ...
  async updateArtefact(artefactData: SingleValueArtefact): Promise<boolean> {
    this.logger.info(
      'Updating single value artefact',
      'ОбновлениеОднозначногоАртефакта',
      {
        model_id: artefactData.model_id,
        artefact_tech_label: artefactData.artefact_tech_label,
    ...
    const { model_id, artefact_tech_label, artefact_string_value, creator } =
      artefactData
    ...
    try {
      ...
      if (!model) {
        this.logger.warn(
          'Model not found for artefact update',
          'МодельНеНайденаДляОбновленияАртефакта',
          {
            model_id
          }
        )
      ...
      this.logger.info(
        'Artefact update parameters resolved',
        'ПараметрыОбновленияАртефактаРассчитаны',
        {
          model_id,
          artefact_id: artefact.artefact_id,
      ...
      if (isMultiFill) {
        ...
        this.logger.info(
          'Skipping artefact update - no changes detected',
          'ПропускОбновленияАртефактаИзмененийНеОбнаружено',
          {
            model_id,
            artefact_tech_label
          }
      ...
      this.logger.info(
        'Single value artefact update completed successfully',
        'ОбновлениеОднозначногоАртефактаУспешноЗавершено',
        {
          model_id,
          artefact_tech_label,
          artefact_id: artefact.artefact_id
      ...
      this.logger.error(
        'Error updating single value artefact',
        'ОшибкаОбновленияОднозначногоАртефакта',
        error,
        {
          model_id: artefactData.model_id,
          artefact_tech_label: artefactData.artefact_tech_label
  ...
  async getArtefactByTechLabel(
    artefact_tech_label: UpdateArtefactDto['artefact_tech_label']
  ): Promise<ArtefactEntity | null> {
    this.logger.info(
      'Getting artefact by technical label',
      'ПолучениеАртефактаПоТехническомуЯрлыку',
      {
        artefact_tech_label,
    ...
    try {
      ...
      if (artefact) {
        this.logger.info(
          'Artefact found successfully',
          'АртефактУспешноНайден',
          {
            artefact_tech_label,
            artefact_id: artefact.artefact_id,
      ...
      this.logger.error(
        ...
        'ОшибкаПолученияАртефактаПоТехническомуЯрлыку',
        error,
        {
          artefact_tech_label
        }
  ...
  async getArtefactValues(
    ...
    try {
      ...
      this.logger.info(
        'Artefact values retrieved successfully',
        'ЗначенияАртефактаУспешноПолучены',
        {
          artefact_id,
      ...
      this.logger.error(
        'Error getting artefact values',
        'ОшибкаПолученияЗначенийАртефакта',
        error,
        {
          artefact_id
        }
  ...
  async getLatestArtefactRealization(
    ...
    this.logger.info(
      'Getting latest artefact realization',
      'ПолучениеПоследнейРеализацииАртефакта',
      {
        model_id,
        artefact_id,
    ...
    try {
      ...
      if (artefactRealization) {
        this.logger.info(
          'Latest artefact realization found',
          'ПоследняяРеализацияАртефактаНайдена',
          {
            model_id,
            artefact_id,
        ...
        this.logger.info(
          'No artefact realization found',
          'РеализацияАртефактаНеНайдена',
          {
            model_id,
            artefact_id
          }
      ...
      this.logger.error(
        'Error getting latest artefact realization',
        'ОшибкаПолученияПоследнейРеализацииАртефакта',
        error,
        {
          model_id,
          artefact_id
  ...
  async getLatestArtefactRealizations(
    ...
    this.logger.info(
      'Getting latest artefact realizations',
      'ПолучениеПоследнихРеализацийАртефакта',
      {
        model_id,
        artefact_id,
        table_name: this.artefactRealizationsTableName
    ...
    try {
      ...
      this.logger.info(
        'Latest artefact realizations retrieved',
        'ПоследниеРеализацииАртефактаПолучены',
        {
          model_id,
          artefact_id,
      ...
      this.logger.error(
        'Error getting latest artefact realizations',
        'ОшибкаПолученияПоследнихРеализацийАртефакта',
        error,
        {
          model_id,
          artefact_id
  ...
  async setEffectiveToArtefactRealization(
    ...
    this.logger.info(
      'Setting effective_to for artefact realization',
      'УстановкаEffectiveToДляРеализацииАртефакта',
      {
    ...
    try {
      ...
      this.logger.info(
        'Effective_to set successfully',
        'EffectiveToУспешноУстановлен',
        {
          model_id: latestArtefactRealization.model_id,
          artefact_id: latestArtefactRealization.artefact_id
        }
      ...
      this.logger.error(
        'Error setting effective_to for artefact realization',
        'ОшибкаУстановкиEffectiveToДляРеализацииАртефакта',
        error,
        {
          model_id: latestArtefactRealization.model_id,
          artefact_id: latestArtefactRealization.artefact_id
  ...
  resolveArtefactValueId(
    ...
    this.logger.info(
      'Resolving artefact value ID',
      'РазрешениеИдентификатораЗначенияАртефакта',
      {
        artefact_string_value: artefactData.artefact_string_value,
        artefact_value_id: artefactData.artefact_value_id,
    ...
    if (!artefactValues) {
      this.logger.info(
        'No artefact values available, returning original value_id',
        'ЗначенияАртефактаНеДоступныВозвратОригинальногоИдентификатора',
        {
          artefact_value_id
        }
      )
    ...
    const valueByString = artefactValues.find(
      (value) =>
        ...
        artefact_string_value.toLowerCase()
    )
    if (valueByString) {
      this.logger.info(
        'Found matching value by string',
        'НайденоСоответствиеПоСтроке',
        {
          artefact_string_value,
    ...
    this.logger.warn(
      'No matching artefact value found, returning original value_id',
      'СоответствующееЗначениеАртефактаНеНайденоВозвратОригинальногоИдентификатора',
      {
        artefact_value_id,
        artefact_string_value
      }
  ...
  async insertArtefactRealization(
    ...
    this.logger.info(
      'Inserting artefact realization',
      'ДобавлениеРеализацииАртефакта',
      {
        model_id,
        artefact_id,
        artefact_value_id,
    ...
    try {
      ...
      this.logger.info(
        'Artefact realization inserted successfully',
        'РеализацияАртефактаУспешноДобавлена',
        {
          model_id,
          artefact_id,
      ...
      this.logger.error(
        'Error inserting artefact realization',
        'ОшибкаДобавленияРеализацииАртефакта',
        error,
        {
          model_id,
          artefact_id,
  ...
  async getMaxArtefactUpdateDate(model_id: string): Promise<any> {
    this.logger.info(
      'Getting max artefact update date',
      'ПолучениеМаксимальнойДатыОбновленияАртефакта',
      {
        model_id
      }
    )
    ...
    try {
      ...
      this.logger.info(
        'Max artefact update date retrieved',
        'МаксимальнаяДатаОбновленияАртефактаПолучена',
        {
          model_id,
      ...
      this.logger.error(
        'Error getting max artefact update date',
        'ОшибкаПолученияМаксимальнойДатыОбновленияАртефакта',
        error,
        {
          model_id
        }
  ...
  canEditArtefact(artefact: ArtefactEntity): boolean {
    ...
    this.logger.info(
      ...
      'ПроверкаВозможностиРедактированияАртефакта',
      {
        artefact_id: artefact.artefact_id,
  ...
  canEditArtefactBySource(
    ...
    this.logger.info(
      ...
      'ПроверкаПравРедактированияАртефактаПоИсточнику',
      {
        artefact_id: artefact.artefact_id,
        artefact_tech_label: artefact.artefact_tech_label,
        is_edit_flg: artefact.is_edit_flg,
    ...
    if (artefact.is_edit_flg === '0') {
      this.logger.info(
        'Artefact editing disabled by flag',
        'РедактированиеАртефактаОтключеноФлагом',
        {
          artefact_id: artefact.artefact_id
        }
      )
    ...
    this.logger.info(
      'Artefact edit permissions determined',
      'ПраваРедактированияАртефактаОпределены',
      {
        artefact_id: artefact.artefact_id,
        ...result,
  ...
  private shouldSkipUpdate(
    ...
    if (!latestArtefactRealization) {
      this.logger.info(
        'No latest realization found, update required',
        'ПоследняяРеализацияНеНайденаТребуетсяОбновление'
      )
      return false
    }
    ...
    if (
      isSelectType &&
      latestArtefactRealization.artefact_value_id === artefact_value_id
    ) {
      shouldSkip = true
      reason = 'artefact_value_id unchanged for select type'
    } else if (
      !isSelectType &&
      latestArtefactRealization.artefact_string_value === artefact_string_value
    ) {
      shouldSkip = true
    ...
    if (shouldSkip) {
      this.logger.info(
        'Skipping artefact update',
        'ПропускОбновленияАртефакта',
        {
          reason,
          is_select_type: isSelectType,
  ...
  private determineFinalStringValue(
    ...
    this.logger.info(
      'Determining final string value',
      'ОпределениеФинальногоСтроковогоЗначения',
      {
        artefact_id: artefact.artefact_id,
        artefact_type_id: artefact.artefact_type_id,
        requires_values: ARTEFACT_TYPES_REQUIRING_VALUES.has(
          artefact.artefact_type_id
        ),
        has_artefact_values: !!artefactValues,
        artefact_value_id,
    ...
    if (
      ARTEFACT_TYPES_REQUIRING_VALUES.has(artefact.artefact_type_id) &&
      artefactValues
    ) {
      const value = artefactValues.find(
        (val) => val.artefact_value_id === artefact_value_id
      )
      const result = value ? value.artefact_value : null

      this.logger.info(
        'Final string value determined from artefact values',
        'ФинальноеСтроковоеЗначениеОпределеноИзЗначенийАртефакта',
        {
          artefact_id: artefact.artefact_id,
          artefact_value_id,
    ...
    this.logger.info(
      'Final string value using original string value',
      'ФинальноеСтроковоеЗначениеИспользуетОригинальноеЗначение',
      {
        artefact_id: artefact.artefact_id,
