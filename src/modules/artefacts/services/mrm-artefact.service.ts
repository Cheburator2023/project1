  ...
  implements IArtefactService
{
  ...
  async handleUpdateArtefact(
    ...
    this.logger.info(
      'Handling update artefact with handlers',
      'ОбработкаОбновленияАртефактаСОбработчиками',
      {
    ...
    if (handler) {
      this.logger.info(
        'Using specific handler for artefact',
        'ИспользованиеСпецифическогоОбработчикаДляАртефакта',
        {
          artefact_tech_label: artefactData.artefact_tech_label,
      ...
      this.logger.info(
        'Using base handler for artefact',
        'ИспользованиеБазовогоОбработчикаДляАртефакта',
        {
          artefact_tech_label: artefactData.artefact_tech_label
        }
      )
    ...
    this.logger.info(
      'Artefact update handling completed',
      'ОбработкаОбновленияАртефактаЗавершена',
      {
        model_id: artefactData.model_id,
        artefact_tech_label: artefactData.artefact_tech_label,
        success: result
