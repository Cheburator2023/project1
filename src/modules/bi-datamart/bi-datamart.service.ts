...
export class BiDatamartService {
  ...
  async syncAllModelsToDatamart(): Promise<{
    ...
    try {
      ...
      this.logger.log('🔍 Вызываем modelsService.getModels({ ignoreModeFilter: true })')
      ...
      for (let i = 0; i < models.length; i += batchSize) {
        ...
        this.logger.log(`🔄 Обрабатываем батч ${Math.floor(i/batchSize) + 1} из ${Math.ceil(models.length/batchSize)} (модели ${i+1}-${Math.min(i+batchSize, models.length)})`)
        ...
        for (const model of batch) {
          try {
            ...
            this.logger.error(`❌ Ошибка при обработке модели ${model.system_model_id}: ${error.message}`)
            result.errors.push(`Модель ${model.system_model_id}: ${error.message}`)
          }
        }
        ...
        const estimatedTimeRemaining = Math.round(avgTimePerModel * remainingModels / 1000)
        
        this.logger.log(`📈 Прогресс: ${processed}/${models.length} (${percentage}%) | Вставлено: ${result.inserted}, Обновлено: ${result.updated}, Пропущено: ${result.skipped} | Осталось ~${estimatedTimeRemaining}сек`)
      }
      ...
      this.logger.log(`📊 Вставлено: ${result.inserted}, Обновлено: ${result.updated}, Пропущено: ${result.skipped}`)
      
      // Подробные логи об измененных моделях
      if (result.insertedModels.length > 0) {
        this.logger.log(`🆕 Новые модели (${result.insertedModels.length}): ${result.insertedModels.join(', ')}`)
      }
      
      if (result.updatedModels.length > 0) {
        this.logger.log(`🔄 Обновленные модели (${result.updatedModels.length}): ${result.updatedModels.join(', ')}`)
      }

    } catch (error) {
      result.duration_ms = Date.now() - startTime
      this.logger.error(`💥 Критическая ошибка синхронизации: ${error.message}`, error.stack)
  ...
  async getDatamartStats() {
    const [totalResult, lastUpdateResult] = await Promise.all([
      this.mrmDatabaseService.query('SELECT COUNT(*) as count FROM models_bi_datamart', {}),
      this.mrmDatabaseService.query('SELECT MAX(updated_at) as last_update FROM models_bi_datamart', {})
    ])
