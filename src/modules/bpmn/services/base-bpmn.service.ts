...
export abstract class BaseBpmnService implements IBpmnService {
  ...
  async getInstances(bpmnIds?: string[]): Promise<BpmnInstanceEntity[]> {
    ...
    try {
      ...
      const result = await this.databaseService.query(query, bpmnIds ? { bpmnIds } : {});

      this.logger.info('BPMN instances retrieved successfully', 'ЭкземплярыBPMNУспешноПолучены', {
      ...
      this.logger.error('Error getting BPMN instances', 'ОшибкаПолученияЭкземпляровBPMN', error, {
        bpmn_ids_count: bpmnIds?.length || 0
      });
      throw error;
    }
