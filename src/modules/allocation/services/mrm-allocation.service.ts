...
export class MrmAllocationService extends BaseAllocationService implements IAllocationService {
  ...
  async handleUpdateAllocation(data: UpdateAllocationDto): Promise<boolean> {
    this.logger.info('Handling update allocation', 'ОбработкаОбновленияРаспределения', data);
    ...
    try {
      ...
      if (!model) {
        this.logger.warn('Model not found for allocation update', 'МодельНеНайденаДляОбновленияРаспределения', {
          model_id
        });
        return false;
      }
      ...
      if (!allocation) {
        this.logger.info('Inserting new allocation', 'ДобавлениеНовогоРаспределения', {
          model_id,
          gbl_id
        });
        ...
        if (!allocationItem) {
          this.logger.error('Failed to insert allocation', 'ОшибкаДобавленияРаспределения', null, {
            model_id,
            gbl_id
          });
          return false;
        ...
        this.logger.info('Updating existing allocation', 'ОбновлениеСуществующегоРаспределения', {
          model_id,
          gbl_id
        });
        ...
        if (!allocationItem) {
          this.logger.error('Failed to update allocation', 'ОшибкаОбновленияРаспределения', null, {
            model_id,
            gbl_id
          });
          return false;
      ...
      if (!allocationHistItem) {
        this.logger.error('Failed to insert allocation history', 'ОшибкаДобавленияИсторииРаспределения', null, {
          allocationId
        });
        return false;
      }

      this.logger.info('Allocation update completed successfully', 'ОбновлениеРаспределенияУспешноЗавершено', {
        model_id,
        gbl_id,
        allocationId
      });
      ...
      this.logger.error('Error handling allocation update', 'ОшибкаОбработкиОбновленияРаспределения', error, data);
      return false;
    }
  }
  ...
  async getAllocation(model_id: UpdateAllocationDto['model_id'], gbl_id: UpdateAllocationDto['gbl_id']): Promise<AllocationEntity | null> {
    ...
    try {
      ...
      this.logger.error('Error getting allocation', 'ОшибкаПолученияРаспределения', error, {
        model_id,
        gbl_id
      });
      throw error;
  ...
  async updateAllocation(data: UpdateAllocationDto): Promise<AllocationEntity | null> {
    ...
    try {
      ...
      if (allocationItem) {
        this.logger.info('Allocation updated successfully', 'РаспределениеУспешноОбновлено', {
          model_id,
          gbl_id
        });
      } else {
        this.logger.warn('Allocation update returned no rows', 'ОбновлениеРаспределенияНеВернулоДанных', {
          model_id,
          gbl_id
        });
      }
      ...
      this.logger.error('Error updating allocation', 'ОшибкаОбновленияРаспределения', error, {
        model_id,
        gbl_id
      });
      throw error;
  ...
  async insertAllocation(data: UpdateAllocationDto): Promise<AllocationEntity | null> {
    ...
    try {
      ...
      if (allocationItem) {
        this.logger.info('Allocation inserted successfully', 'РаспределениеУспешноДобавлено', {
          model_id,
          gbl_id,
          allocation_id: allocationItem.allocation_id
        });
      } else {
        this.logger.warn('Allocation insert returned no rows', 'ДобавлениеРаспределенияНеВернулоДанных', {
          model_id,
          gbl_id
        });
      }
      ...
      this.logger.error('Error inserting allocation', 'ОшибкаДобавленияРаспределения', error, {
        model_id,
        gbl_id
      });
      throw error;
  ...
  async insertAllocationHistory(allocation_id: any, data: UpdateAllocationDto): Promise<AllocationEntity | null> {
    const { model_id, gbl_id, percent, comment, creator } = data;

    this.logger.info('Inserting allocation history', 'ДобавлениеИсторииРаспределения', {
      allocation_id,
      model_id,
      gbl_id
    });
    ...
    try {
      ...
      if (allocationHistItem) {
        this.logger.info('Allocation history inserted successfully', 'ИсторияРаспределенияУспешноДобавлена', {
          allocation_id
        });
      } else {
        this.logger.warn('Allocation history insert returned no rows', 'ДобавлениеИсторииРаспределенияНеВернулоДанных', {
          allocation_id
        });
      }
      ...
      this.logger.error('Error inserting allocation history', 'ОшибкаДобавленияИсторииРаспределения', error, {
        allocation_id
      });
      throw error;
    }
