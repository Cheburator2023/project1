const getModels = `
SELECT m.model_id AS system_model_id,
       m.model_name,
       m.model_version,
       m.create_date,
       m.update_date,
...
FROM models_new m
    ...
    WHERE (muh.confirmation_quarter IN (1, 2, 3)
        AND muh.confirmation_year = EXTRACT(YEAR FROM CURRENT_DATE))
       OR (:use_previous_year_for_q4 = TRUE AND muh.confirmation_quarter = 4
        AND muh.confirmation_year = EXTRACT(YEAR FROM CURRENT_DATE) - 1)
       OR (:use_previous_year_for_q4 = FALSE AND muh.confirmation_quarter = 4
        AND muh.confirmation_year = EXTRACT(YEAR FROM CURRENT_DATE))
    ...
    GROUP BY mah.model_id
         ...
         LEFT JOIN (
    SELECT ar.model_id                                                                                                                    AS artefacts_model_id,
           STRING_AGG((CASE WHEN ar.artefact_id = 2032 THEN av.artefact_value ELSE NULL END)::Varchar, ','
                      ORDER BY ar.artefact_value_id)                                                                                      AS business_customer_departament
...
WHERE (m.temp_block_flag != 1 OR m.temp_block_flag IS NULL)
