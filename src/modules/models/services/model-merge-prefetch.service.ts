...
type ArtefactSnapshot = Record<string, { artefact_string_value: string | null; effective_from: string | null }>
...
export class ModelMergePrefetchService {
  ...
  async buildMergeMaps(
    ...
    let sumRows: Array<{ model_id: string; artefact_id: number; artefact_string_value: string | null; effective_from: string | null }> = []
    let mrmRows: Array<{ model_id: string; artefact_id: number; artefact_string_value: string | null; effective_from: string | null }> = []

    if (modelIds.length && (sumIds.length || mrmIds.length)) {
      const [sum, mrm] = await Promise.all([
        sumIds.length ? this.sumDatabaseService.query(getLatestSumArtefacts, { artefact_ids: sumIds, model_ids: modelIds, filter_date: filterDate }) : Promise.resolve([]),
        mrmIds.length ? this.mrmDatabaseService.query(getLatestSumRmArtefacts, { artefact_ids: mrmIds, model_ids: modelIds, filter_date: filterDate }) : Promise.resolve([])
      ])
    ...
    for (const r of sumRows) {
      sumMap[`${r.model_id}:${r.artefact_id}`] = { artefact_string_value: r.artefact_string_value, effective_from: r.effective_from }
    }
    const mrmMap: ArtefactSnapshot = Object.create(null)
    for (const r of mrmRows) {
      mrmMap[`${r.model_id}:${r.artefact_id}`] = { artefact_string_value: r.artefact_string_value, effective_from: r.effective_from }
    }
