...
export abstract class BaseAssignmentService implements IAssignmentService {
  ...
  async getAssignmentsWithRolesByModelId(
    ...
    this.logger.info('Getting assignments with roles by model ID', 'ПолучениеНазначенийСРолямиПоИдентификаторуМодели', {
      model_ids_count: modelIds?.length || 0,
    ...
    try {
        ...
        FROM ASSIGNEE_HIST AH
          ...
          LEFT JOIN (
            ...
            FROM (
              SELECT MODEL_ID,
                     BPMN_KEY_DESC,
                     ROW_NUMBER() OVER (PARTITION BY BI.model_id ORDER BY bi.effective_to DESC, bi.effective_from DESC, bi.bpmn_key_id DESC) AS rn
      ...
      this.logger.info('Assignments with roles retrieved successfully', 'НазначенияСРолямиУспешноПолучены', {
        assignments_count: result.length,
        model_ids_count: modelIds?.length || 0
      });

      return result;
    } catch (error) {
      this.logger.error('Error getting assignments with roles', 'ОшибкаПолученияНазначенийСРолями', error, {
        model_ids_count: modelIds?.length || 0
      });
      throw error;
    }
