...
export abstract class BaseAssignmentService implements IAssignmentService {
  ...
  async getAssignmentsWithRolesByModelId(
    ...
    this.logger.info(
      'Getting assignments with roles by model ID',
      'ПолучениеНазначенийСРолямиПоИдентификаторуМодели',
      {
    ...
    try {
      ...
      this.logger.info(
        'Assignments with roles retrieved successfully',
        'НазначенияСРолямиУспешноПолучены',
        {
      ...
      this.logger.error(
        'Error getting assignments with roles',
        'ОшибкаПолученияНазначенийСРолями',
        error,
        {
          model_ids_count: modelIds?.length || 0
        }
