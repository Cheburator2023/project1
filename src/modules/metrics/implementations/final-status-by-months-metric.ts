...
import {
  BaseMetricResult,
  FinalStatusByMonthModelsMetricResult,
  IFinalStatusMetric
} from '../interfaces'
...
export class FinalStatusByMonthsMetric extends IndependentMetric<FinalStatusByMonthModelsMetricResult> {
  ...
  private filteredModelsByMonth: {
    [month: number]: { model: any; date: Date }[]
  } = {}
   ...
   */
  private getModelsWithCurrentYearDates(
    models: any[],
    year: number
  ): Set<string> {
    ...
    models.forEach((model) => {
      ...
      if (
        modelDates.some(
          (date) => date >= currentYearStart && date <= currentYearEnd
        )
      ) {
  ...
  calculate(): FinalStatusByMonthModelsMetricResult {
    ...
    this.finalStatusMetric = new FinalStatusMetric(
      MetricsEnum.FinalStatusModelsMetric
    )
    this.finalStatusMetric.initialize(this.models, this.startDate, this.endDate)
    ...
    const months: FinalStatusByMonthModelsMetricResult = Array.from(
      { length: 12 },
      (_, index) => 0
    ) as FinalStatusByMonthModelsMetricResult
    ...
    if (!this.startDate && !this.endDate) {
      ...
      const januaryModels = currentYearModels.filter(
      ...
      for (let month = 1; month < 12; month++) {
        const monthEntries = currentYearModels.filter(
      ...
      this.filteredModelsWithDates = this.finalStatusMetric
        .filterModels(this.models, this.startDate, this.endDate, false, true)
        .filter(({ date }) => {
