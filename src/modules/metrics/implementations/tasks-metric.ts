...
export class TasksMetric extends IndependentMetric<TasksMetricResult> {
  ...
  private groupTasksByRole(
    tasks: TaskMetricItem[]
  ): Record<USER_ROLES, TaskMetricItem[]> {
  ...
  private flattenGroupedTasks(
    tasksByRole: Record<USER_ROLES, TaskMetricItem[]>
  ): TaskMetricItem[] {
    return Object.values(tasksByRole).flat()
  }

  private buildResultFromGroupedTasks(
    groupedTasks: Record<USER_ROLES, TaskMetricItem[]>
  ): TasksMetricResult {
