...
export class RegistryCoverageMetric extends DependentMetric<
  RegistryCoverageResult,
  Pick<
    MetricDependencyMap,
    | MetricsEnum.ImplementedModelsMetric
    | MetricsEnum.DevelopedModelsMetric
    | MetricsEnum.MrmModelsMetric
  >
> {
  calculate(): RegistryCoverageResult {
    // Summing up the total count and delta for all models
    const { count: totalModelsCount, delta: totalModelsDelta } = Object.keys(
      this.dependencies
    ).reduce(
