...
export interface IIndependentMetric<T extends BaseMetricResult> extends IMetric<T> {
  initialize(models: any[], startDate: string | null, endDate: string | null): void;
}

// Интерфейс для зависимой метрики
export interface IDependentMetric<T extends BaseMetricResult, D extends Partial<MetricDependencyMap>> extends IMetric<T> {
  initialize(
    models: any[],
