...
import { isOnMonitoringModels as isOnMonitoringSumModels } from './sql/sum';
import { isOnMonitoringModels as isOnMonitoringSumRmModels } from './sql/sum-rm';

@Injectable()
export class IsOnMonitoringService {
  constructor(
  ...
  private filterByStreams(
    ...
    return filteredStreams.some((filteredStream) =>
      streams.includes(filteredStream),
    );
  }
  ...
  private async getModels(
    ...
    const sumRawData = await this.sumDatabaseService.query(
      isOnMonitoringSumModels,
      {},
    );
    const sumRmRawData = await this.mrmDatabaseService.query(
    ...
    const filteredData = mergedData.filter(
      (item) =>
        this.filterByStringDate(item.value, startDate, endDate) &&
  ...
  async metric(
    ...
    const deltaCount = this.calculateDeltaByStringValue(
      models,
      startDate,
      endDate,
    );
