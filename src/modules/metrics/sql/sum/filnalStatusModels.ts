...
RankedArtefacts AS (
    ...
    WHERE a.artefact_tech_label IN ('rs_model_decommiss_date', 'date_of_introduction_into_operation', 'model_epic_05_date', 'developing_end_date', 'data_completion_of_stage_05a', 'Departament', 'automl_flg')
)
SELECT m.model_id,
       coalesce(
...
LEFT JOIN RankedArtefacts AS ar4
  ON m.model_id = ar4.model_id AND ar4.artefact_id = (SELECT artefact_id FROM artefacts WHERE artefact_tech_label = 'developing_end_date') AND ar4.rn = 1
LEFT JOIN RankedArtefacts AS ar5
  ON m.model_id = ar5.model_id AND ar5.artefact_id = (SELECT artefact_id FROM artefacts WHERE artefact_tech_label = 'data_completion_of_stage_05a') AND ar5.rn = 1
...
LEFT JOIN RankedArtefacts AS ar7
  ON m.model_id = ar7.model_id AND ar7.artefact_id = (SELECT artefact_id FROM artefacts WHERE artefact_tech_label = 'automl_flg' AND artefact_id = 310) AND ar7.rn = 1
WHERE coalesce(
    ar1.artefact_string_value,
    ar2.artefact_string_value,
    ar3.artefact_string_value,
...
AND (ar7.artefact_string_value IS NULL OR ar7.artefact_string_value != 'true')
