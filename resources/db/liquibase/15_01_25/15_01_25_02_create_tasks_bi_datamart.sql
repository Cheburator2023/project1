...
CREATE TABLE IF NOT EXISTS tasks_bi_datamart (
    -- Автоинкрементный уникальный идентификатор записи (для поддержки дублей)
    id SERIAL PRIMARY KEY,
    
    -- Основные поля задачи (могут дублироваться)
...
CREATE INDEX idx_tasks_bi_datamart_task_id ON tasks_bi_datamart(task_id);
CREATE INDEX idx_tasks_bi_datamart_model_id ON tasks_bi_datamart(model_id);
...
CREATE INDEX idx_tasks_bi_datamart_task_data_gin ON tasks_bi_datamart USING GIN (task_data);
...
-- Композитный индекс для быстрого поиска по task_id + model_id (не уникальный!)
CREATE INDEX idx_tasks_bi_datamart_task_model ON tasks_bi_datamart(task_id, model_id);

-- Комментарии
COMMENT ON TABLE tasks_bi_datamart IS 'BI витрина для агрегированных данных по задачам (поддерживает дубли)';
COMMENT ON COLUMN tasks_bi_datamart.id IS 'Автоинкрементный уникальный идентификатор записи';
